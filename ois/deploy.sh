...
jq -e '.harness == "claude" and (.exec | index("claude") == 0) and ((.exec // []) | index("--dangerously-load-development-channels") != null) and .pluginSpec == "plugin:agent-adapter@agentic-network" and .configDirTemplate == "~/.config/apnex-agents/{agent}.claude" and .exportsRetiredInstanceId == false and .envTemplate.WORK_DIR == "{workspace}" and .envTemplate.HUB_LLM_MODEL == "{hubModelTag}" and (.envTemplate | has("OIS_INSTANCE_ID") | not) and .credentialImports.anthropic.inject == "env" and .credentialImports.anthropic.name == "CLAUDE_CODE_OAUTH_TOKEN" and .claudeSettings.model == "opus" and .claudeSettings.enableWorkflows == true and .claudeSettings.ultracode == true and .claudeSettings.includeCoAuthoredBy == false' "$CLAUDE_HARNESS_CONFIG_SRC" >/dev/null || { echo "error: repo claude harness config lacks required centralized Claude launch/settings policy — refusing to deploy" >&2; exit 1; }

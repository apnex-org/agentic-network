...
- Add `RepoEventBridgeSubstrateAdapter` at **`hub/src/storage-substrate/repo-event-bridge-adapter.ts`** (v1.1 W3-dispatch-prep fold per thread-579 round 5 B7-class fold; was `packages/repo-event-bridge/src/substrate-adapter.ts` in v1.0) — wraps HubStorageSubstrate's CAS primitives + maps path-shape to (kind, id) shape; presents StorageProviderWithTokenRead-compatible interface for cursor-store.ts narrow-typed consumption. **Rationale for hub-side location:** repo-event-bridge package can't cleanly import HubStorageSubstrate types without cross-package boundary (tsconfig `rootDir` violations); adapter is hub-internal-glue (wires RepoEventBridge constructor at hub/src/index.ts:840; not a repo-event-bridge concern); repo-event-bridge stays substrate-agnostic consuming only StorageProviderWithTokenRead-narrowed interface. W0.4 spike (commit a3accb9) verified zero-blocker for W3 commitment.

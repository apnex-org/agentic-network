...
| `pack_gate` | **verifier-gate** | verifier | assemble_pack | — | `ev_pack_verified`·review (refResolvable, producedBy=verifier) |
...
- The verifier-gate path **is** buildable: a `review`-kind requirement with `refResolvable` resolves the gate WorkItem and checks its Hub-stamped `createdBy.role === "verifier"` (non-spoofable; repo-substrate L962–971), and a non-refResolvable review falls back to a `producedBy`→`role=verifier` registry check (L973–981). `pack_gate` / `verify_ranking` as verifier-gates with review evidence is real.

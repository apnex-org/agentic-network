...
**Schema-validated JSON manifest (harness STANDARD) + ENV (per-agent INSTANCE)** — mirrors scion's YAML+env split, JSON because the kernel is JS and the manifest co-locates with the typed hooks. Manifest fields: command/env-template/**capability-matrix (3-valued yes/partial/no + REASON + per-capability unevenness** — the scion-steelman discipline)/auth-order/injection-flags/dialect. **Per-agent config split** (steve F1, audit-5242 — aligns §3 with §5): **non-secret instance config via ENV** (identity name/role + Hub URL + worktree/branch + model/quota) is DISTINCT from **secrets, which are FILE-MOUNTED, not env** (`OIS_HUB_TOKEN` + the LLM `CLAUDE_CODE_OAUTH_TOKEN` + git creds — per §5). The ENV may carry secret PATHS/pointers, NEVER raw secret values (scion-avoid[10]; no secret in `docker inspect`/`/proc`). References-as-data, not topology.
...
- **L1.5 — liveness self-watchdog *(NEW — closes the hole)*:** an **application-level session-validity probe independent of transport keepalive** (e.g. the injection consumption-ACK round-trip, or a periodic Hub round-trip). On failure to confirm a live Hub session within a bounded budget → **the kernel (a CLI GRANDCHILD — it CANNOT exit PID-1 directly) writes a wedged-restart sentinel (`/run/adapter-wedged`, exit-code-75 semantics) + self-exits the shim; the PID-1 thin supervisor watches the sentinel → exits the container** → docker-L2 restarts → re-handshake fresh (steve F2, audit-5242 + the P1c sentinel-contract: the out-of-band grandchild→supervisor signal IS the EMBEDDED exit-propagation seam — P1c emits it, P1e's supervisor consumes it). This converts the *undetected* live-wedge into a **container**-exit L2 can act on.
...
- **Build environment (Director-confirmed 2026-06-29):** build the image on **Google Cloud Build** — our established image-build infra; modern buildkit (0.11+) makes `SOURCE_DATE_EPOCH`/reproducible builds achievable, so it sidesteps the local `docker 20.10.3` cap on byte-identical digests (surfaced at P1a). Reuse the existing Cloud Build config + the scion-adopt Cloud-Build build/push-split + layer-cache shape (audit D-img). P1a's digest-equality test runs against the Cloud Build output; any residual non-determinism is surfaced, not faked.
...
- **P1e — containerise on docker-compose** + L2 restart-supervision + **host-side worktree creation mounted-in (FORBID in-container worktree creation** — the `--relative-paths` path-identity hazard; isolation-by-absence; scion avoid[9] fold) + **prune the image to the adapter dep-closure** (P1a's repro image copies the WHOLE monorepo `node_modules` — `google-auth-library`/`jose`/`@google-cloud/*` from other workspaces → bloat; P1e prunes to only the `@apnex` kernel + claude-shim runtime deps; scion-adopt "keep the base minimal"; greg-surfaced at P1a 2026-06-29).

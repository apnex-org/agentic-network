...
- **T5 (cold-start spine) — CONVERGED: REQUIRED.** `get_next` + non-dark empty digest + legal-moves + schema-enforced runbook. **Pre-read/inter-node-artifact accessibility is a blueprint VALIDATION rule** (seed-time, fail-closed): every referenced input + evidence-artifact must resolve **inline in payload / a pinned git ref / a discoverable Hub Document** — never a dangling `docs/...` path. (Proven necessary: the council hit this **3×** live.) **Director-extension (2026-06-28) — the spine also covers the ARCHITECT's own idle loop:** re-engagement via `get_next`/claimable-digest (push) + heartbeat via the lease-stall-warning (push, fires ~6 min pre-expiry — demonstrated live), retiring the ad-hoc ScheduleWakeup timer-poll AND **FR-38** (the lease-renewal-race). Push (Hub) subsumes poll for both halves; the only residual poll, if ever needed, is delivered by the **local adapter as a first-class primitive**, not improvised per-agent. Authoritative cross-scenario design deferred to **idea-382 (M-Wake-Primitives-Unification)**; stint-5 S2 applies the principle to the architect loop (timer → long fallback, push-primary).
...
- **Non-dark empty digest** (SEVERE→fixed). `list_ready_work`/`get_next` always carry `reason`+`nextAction` when empty (capped → "renew_lease or complete your N in-progress"; quarantined → "call clear_work_quarantine"; genuinely-empty → "no ready work in role X; stint at phase Y awaiting gate Z by role W"). Empty is still an instruction.
  - ⚠️ **CORRECTION 2026-07-28 (`work-593` / idea-675) — READ BEFORE BUILDING THIS.** The Director-dialogue text above is kept verbatim (it is a record of what was agreed, and rewriting it would falsify that record), **but unlike the other two superseded references this one is ACTIVE GUIDANCE FOR UNBUILT WORK — anyone implementing `get_next` from this line would ship a reason code and a verb that no longer exist.** The `quarantined` branch is **GONE**: `ReadyEmptyReason` is now exactly `wip_capped | no_claimable_ready`, and `clear_work_quarantine` is unregistered. **The principle survives untouched and is the part that mattered — an empty digest must still explain itself and must never be a dark zero.** Only the third arm of the example is stale. See `docs/verification/work-593-quarantine-removal.md`.

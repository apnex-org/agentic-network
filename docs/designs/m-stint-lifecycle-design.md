...
- **Triangulation was ORGANIC, not REQUIRED (Director-surfaced gap, 2026-06-28).** The position×lens briefs did not require triangulation against project entities — they carried `{topic, position, lens}` + "read the pre-read." Participants organically grounded in bugs / ideas / friction(FR) / PRs / calibrations (well — bug-137, bug-190/work-44, FR-27/36/38, idea-356/357, #410), **but teles and axioms were under-cited** — the exact classes a required field would have forced (organic triangulation covers what's salient in the pre-read; it misses the teles the work serves + the axioms it must honor). **→ seed_blueprint / council-blueprint REQUIREMENT:** a position/clash brief carries a mandatory `triangulateAgainst: [bugs, ideas, friction, calibrations, tele, axioms]` directive, and the node's `evidenceRequirements` checks for specific entity-ID citations per assigned position (a position with zero cited entities fails the gate). This mechanizes *rigor*, not just structure — aligns mission-kit **M1 triangulated-review** + calibration **#85 ground-truth-over-assumption** + the **A-series axioms** (positions must name the axioms they rest on / risk). The council mechanism is incomplete without it.

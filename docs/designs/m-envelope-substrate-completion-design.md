# M-Envelope-Substrate-Completion — Design v1.3

**Status:** v1.3 — W2 finding-A: `renameMap` completed from renames-only to the full bare→envelope FIELD-MOVEMENT authority (renames + partition-relocations, **49 entries / 21 kinds**), test-enforced by the W2 sentinel-probe faithfulness oracle + classification invariant (§2.6); bug-147 (latent post-W6 idempotency-break) fixed; bug-148 (Notification phantom) + bug-149 (JSONB-index W6-gate) filed. Prior: v1.2 — Phase 6 preflight measurement corrections folded (thread-658 converged): (c1) live bare-shape writers — "cutover @100%" struck; (c2) dirty-cursor trap — W6 cursor discipline + W1 boot-put slice; downtime MEASURED 47-57s composite (<60s confirmed). Prior: v1.1 thread-657 audit integration + Director idea-318 scope-in + W5 self-trigger fold. **mission-90 ACTIVE; W1 (`0ba9707`) + W2 (`483cbf4`) MERGED, W3 (task-417) in-flight; Phase 8 open.**
...
Tele alignment (whole-mission; **PINNED at Phase 5 Manifest 2026-06-10** per `docs/methodology/tele-glossary.md`): primary **tele-1 (Sovereign State Transparency)** — the state-backplane must return true query results; envelope-blind filters violate "no functional unit possesses opaque truth" at the read surface — + **tele-7 (Resilient Agentic Operations)** — "no silent failures" verbatim; bug-138 is the silent-failure class. Secondary: **tele-2 (Isomorphic Specification)** — the renameMap-at-SchemaDef declarative runtime contract is spec-is-system — + **tele-6 (Frictionless Agentic Collaboration)** — operator/agent trust in the daily-driver list tools; Survey + ledger-reconciliation flows regain trustworthy results (substrate-self-dogfood). *(Survey-era placeholder short-names structural-elimination/substrate-fidelity/anti-silent-failure map to tele-1/tele-7 as above.)*
...
### §2.6 renameMap inventory — completed at W2 to the full field-movement authority: 49 entries / 21 kinds (renames + relocations) (F3; A3 RESOLVED)
...
**⊕ W2 finding-A amendment (2026-06-19, completed @ `483cbf4`):** the §2.1 "generic for ANY key" contract was completed from RENAMES-only to the full bare→envelope FIELD-MOVEMENT authority — renames AND partition-relocations (e.g. `severity→spec.severity`, `class→spec.class`, `idempotencyKey→metadata.idempotencyKey`, Message `delivery→spec.delivery` / `scheduledState→status.scheduledState`). **Total 49 entries / 21 kinds**, all-schemas-only (migration modules untouched — the encoder places relocations via the partition). The authoritative inventory is `all-schemas.ts`, **test-enforced by the W2 sentinel-probe faithfulness oracle** (every entry asserted against the real `migrateOne` placement) **+ a classification invariant** (every filterable moved field is renameMap-covered OR documented-excluded — cascade-keys handled by repo envelope-first dual-path with the W1 null-pin; `Notification.recipientAgentId` phantom per bug-148; structural-transforms like `tags→metadata.labels` array→map). Counting is NOT the discipline — the oracle is. **The renames table below is the FSM/rename SUBSET (historical, thread-657); see `all-schemas.ts` for the complete authority.** Motivating catch: bug-147 — a latent post-W6 idempotency-break the rename-only gap would have shipped silently.
...
- **idea-318 (Reconciler Status-Write loop) → W5 — DIRECTOR SCOPE-IN OVERRIDE (2026-06-10; supersedes the thread-657-converged demote).** idea-318 is the reconciler status-WRITE loop: per reconcile cycle, write `status.phase` / `appliedVersion` / `reconcileError` to the SchemaDef entities themselves. `schema-reconciler.ts` currently has NO write/validate/put-status path — this is **NET-NEW work, NOT satisfied by W1+W2** (the v0.1 "mechanically satisfied at W1+W2 — no separate wave" claim is CORRECTED). It gets its OWN wave (W5) with its own release-gate, depending on W1 only. *Provenance:* thread-657 converged on demoting idea-318 out of scope; the Director exercised gate-authority to scope it IN (2026-06-10), preserving the Survey Q5(a) maximal-fold. **W5 focused re-audit (engineer, 2026-06-10, post-v1.0):** surfaced the CRITICAL self-trigger constraint — the status-write fires the reconciler's own watch-loop; a literal per-cycle write infinite-loops at runtime. W5 ships converge-then-stop write semantics + a runtimeLoop spec-equality guard, gated by bounded-storm + reinstated 3× restart-cycle + restart-survival tests (full spec: §4 W5 row). Boot-failure posture WARN (Director-dispositioned). This deliberately deviates from idea-318's literal "per-cycle timestamp refresh" framing; the deviation is flagged in the W5 task description.
...
**The 2026-05-25 cutover migrated the then-existing rows — but prod has LIVE bare-shape WRITERS (preflight c1, thread-658, measured on a prod-snapshot clone 2026-06-10):** ~790 non-envelope rows and growing — Message 504 (50 written the day of measurement), Audit 214, SchemaDef 23 (boot-put re-bares every restart — killed at source by the W1 boot-put slice), MigrationCursor 22 (no module, by-design), Bug 9, PendingAction 7, Thread 4, Idea 4, Mission 2, Task 1. The v1.1 "cutover @100% / residual ≈0" framing is STRUCK — write-paths for ~8 kinds still produce legacy-flat rows (wild dispositive: `get_idea` returns idea-318/320 enveloped but idea-323/324 flat). **Consequences:** (i) closing the bare writers IS idea-324's W4 surface — the W4 gate mandates close-ALL-bare-writers + a no-new-bare-rows canary (write through every tool path post-W4, assert envelope lands); (ii) W6-prep re-verifies writer-closure (zero bare rows newer than W4-ship) before cutover; (iii) the W6 re-migration is a real multi-hundred-row operation, not a no-op — still comfortably inside the measured envelope (686 rows in 12.9s at preflight). Q3a then re-migrates all residual rows and flips the reader strict (envelope-only), retiring the `SUBSTRATE_ENVELOPE_TOLERANT` tolerant reader-parse (`hub/src/index.ts:126` + `shape-helpers.ts:4-13` + `agent-envelope-shape.ts` — the idea-320 surface; W6 flips, W8 deletes). There is NO legacy OR-branch in `translateFilterClause` to delete — single envelope-blind path (code-verified at thread-657). **Harness consequence:** re-migration convergence AND silent translation-miss are BOTH live risks now; the shadow harness validates **renameMap-translation CORRECTNESS** (§3.3) while the W6 cursor-discipline (§3.2) owns convergence. "Redesign over perpetual-accommodation" (methodology #25). Data-touch confined to W6; W1–W4 pure read-path (plus the W1 boot-put-shape fix); W5 writes reconcile-status to SchemaDef entities only.
...
W6 sequence:
1. **W6-prep gate (pre Hub-stop):** W1–W5 integration green at HEAD; measured re-migration time on a prod snapshot within budget; shadow-read harness PASSED offline (§3.3); rollback rehearsed on the clone; KINDS-array reconciliation done; **writer-closure re-verified (zero bare rows newer than W4-ship — preflight c1)**; **stuck-Message-40 root-caused (preflight c2: 40 Message rows survived 4 convergence runs with errored=0 — silent decoder-skip suspect; errored=0 is itself a finding)**.
2. **Hub stop + pre-cutover snapshot** — `pg_dump -Fc` via `scripts/local/hub-snapshot.sh` as the during-cutover-abort safety net.
3. **Re-migration with cursor-discipline (preflight c2 — PROVEN dirty-cursor trap):** `resetCheckpoint` ALL kinds before the run (`MigrationCursorRepository.resetCheckpoint:89` — the cursor's lexical checkpoint-skip permanently blinds post-cutover rows whose ids sort before the stored checkpoint, e.g. `"bug-137" < "bug-99"` string-ordered); then **loop `run-envelope-migration.ts` until migrated=0** (single-pass under-delivers on dirty state — clone empirics: dry-run predicted 731, run-1 migrated 686, run-2 found 5 more, runs 3-4 converged).
4. **Verification** — per-kind count parity + content-hash spot-check + zero legacy-shape rows **outside the exemption set** (exemptions: MigrationCursor 22 — no migration module, by-design; SchemaDef expected ZERO post-W1 boot-put fix — if bare SchemaDef rows appear, the W1 fix regressed).
...
**Downtime budget (A7 RESOLVED; MEASURED at preflight 2026-06-10):** a single **<60s TOTAL OBSERVED DOWNTIME** target — **CONFIRMED-ACHIEVABLE at 47-57s composite** on a real prod-snapshot clone (pg_dump 20.8s for 24,397 entities/12MB — post-Hub-stop, in-window; pg_restore 2.0s; envelope-migrate first-run 12.9s/686 rows; verification <1s; + 12-22s runbook lifecycle bookends per `hub-storage-cutover-runbook.md:52`). **Margin is THIN (3-13s):** image pre-pull is MANDATORY (watchtower keeps it current); the §R11 parallel-per-kind lever stays in reserve. Shadow-read runs OFFLINE on the clone (§3.3) — off the downtime critical path. Dump artifact retained (`/tmp/m90-preflight.dump`, local + hub-vm) for harness corpus reuse.
...
**Purpose: renameMap-translation CORRECTNESS** — convergence is owned separately by the W6 cursor-discipline (§3.2 step 3; the §3.1 live-writer correction means residual is ~hundreds, not ≈0). **Runs OFFLINE on a throwaway clone — off the downtime critical path.**
...
|---|---|---|---|---|
| **W1** | renameMap runtime contract: promote `renameMap` to `SchemaDef` + reconciler translation table (§2.1/§2.2). Populate all 23 runtime SchemaDef consts in `schemas/all-schemas.ts` — 20 carry renameMap; Counter / Document / MigrationCursor none (corrects the v0.1 "all 22"). **PLUS (preflight c2 fold): boot-put envelope-correctness** — the reconciler boot put writes bare `{id, ...def}` (`schema-reconciler.ts:111`), re-baring all 23 SchemaDef rows every restart; make it envelope-correct at W1 (kills the W6 SchemaDef exemption at source + removes the W5 boot-clobber hazard; coordinate put-shape with W5's status-MERGE requirement). | `storage-substrate/types.ts` (SchemaDef + RenameMap); `schema-reconciler.ts` (cache + `getFieldTranslation`; `buildFieldTranslationMap` positioned for failure-propagation §2.2; boot-put shape); `schemas/all-schemas.ts` (20 renameMap blocks; `SchemaDefMeta` :352 carries `kind→metadata.name`) | CI green; **zero-index-churn CODE-PROVEN per §2.2** (field-read-set argument; 3× restart-cycle retained as regression-guard only); malformed-renameMap throw propagates to `start()`'s failure collector; cache unit tests pinned **per-kind-EXACT to the §2.6 inventory (never to a count)** — incl. collision (Message.kind→metadata.messageKind) + opaque (body→status.cursor) + no-op for unmapped keys; **SchemaDef rows envelope-correct across restarts (boot-put dispositive)** | — (gates all) |
...
| **W4** | Repo / sweeper / watch sweep (idea-324 surface). MUST include the watch-path `matchesFilter` fix in **BOTH substrates**: `postgres-substrate.ts:496` (called :224, :304) + `memory-substrate.ts:496-542` — N1 load-bearing: sweepers consuming `substrate.watch` hit the SAME bug-138 silent-miss. Repositories internally compose substrate; task-internal entity-reads normalize envelope-shape. Handler surface UNCHANGED. **PLUS (preflight c1, thread-658): CLOSE ALL LIVE BARE-SHAPE WRITERS** — ~8 kinds still write legacy-flat rows (~790 rows at preflight, 50 Messages/day growth; Message 504, Audit 214, Bug 9, PendingAction 7, Thread 4, Idea 4, Mission 2, Task 1 + SchemaDef boot-put handled at W1). | `hub/src/entities/*-repository*.ts`; `hub/src/sweepers/`; `postgres-substrate.ts:496`; `memory-substrate.ts:496-542`; every legacy-flat write-path (preflight c1 inventory) | CI green; sweeper `substrate.watch` e2e with renamed-key filters (BOTH substrates); repository CAS via substrate API; existing I*Store integration green; **no-new-bare-rows CANARY — write through EVERY tool path post-W4 + assert envelope-shape lands (per-kind)**; **W1–W4 suite proves W1–W3 correctness WITHOUT W4 present** (F8/R7) | W1–W3 |
| **W5** | idea-318 reconciler status-WRITE loop (**Director scope-in 2026-06-10**, §2.8): write `status.phase` / `appliedVersion` / `reconcileError` to SchemaDef entities. NET-NEW — no write/validate/put-status path today. **CRITICAL self-trigger constraint (W5 re-audit 2026-06-10, code-proven):** the status-write put fires the reconciler's OWN watch-loop (`runtimeLoop` re-reconciles on EVERY SchemaDef put, `schema-reconciler.ts:261-265`, no spec-change guard; NOTIFY trigger has no OLD-vs-NEW guard, `002-notify-trigger.sql:22-24`; `substrate.put` bumps resource_version unconditionally, `postgres-substrate.ts:92-98`) — a literal per-cycle write infinite-loops at runtime (boot is safe: LISTEN starts post-boot-loop at :137). W5 therefore ships BOTH: **(i) converge-then-stop write semantics** — put ONLY on MATERIAL status change (phase/appliedVersion/reconcileError differ); NO `lastReconciledAt`-style refresh on no-op cycles (deliberate, flagged deviation from idea-318's literal per-cycle-timestamp — a data-changed guard alone does NOT terminate if a timestamp refreshes every cycle); **(ii) spec-equality guard in `runtimeLoop`** — skip re-reconcile when spec-relevant fields (kind/version/fields/indexes/indexOwnershipPattern/renameMap) are unchanged vs a cached per-kind copy (also kills today's wasteful no-op re-reconciles). **Boot-shape:** boot put writes bare `{id, ...def}` with NO status (`schema-reconciler.ts:111`) — the W5 write MERGES into the freshly-put row. **Boot-failure posture: WARN (Director-dispositioned 2026-06-10)** — status-write failure must NOT fail Hub start; `reconcileError` is itself the failure-surface (tele-7: loud, not fatal). | `schema-reconciler.ts` (status-write path + runtimeLoop spec-equality guard) | **OWN release-gate:** reconcile cycle observably writes `status.phase`/`appliedVersion` on success + `reconcileError` on failure; **bounded-storm test** — one SchemaDef version-bump with the write active asserts reconcile-cycle count exactly N (1 real + ≤1 echo), NOT unbounded; **3× restart-cycle test REINSTATED LOAD-BEARING** (A2's static zero-churn proof covered a read-only reconciler — W5's write-back invalidates that half; assert bounded cycles + zero index-DDL churn WITH the write active); **restart-survival** — write → restart → status re-converges (not lost to the boot bare-def clobber); CI green; independently revertible without disturbing W2–W4 | W1 only (file-collocation + merge-ordering; correctness-independent) |
| **W6** | Reversible re-migration + shadow-validation + envelope-only strict-flip (§3). **REUSES `hub/src/scripts/run-envelope-migration.ts` (`npm run envelope-migrate`)** — the v0.1-planned NEW migrate script is DROPPED. Pre-reuse: reconcile the cutover script's KINDS array via the single-authority `--list-kinds` derivation from `registeredKinds()` (hand-add REJECTED — re-seeds dual-source drift; the array currently lists 21, missing Notification). **Cursor discipline (preflight c2 — PROVEN lexical checkpoint-skip trap): `resetCheckpoint` ALL kinds pre-run + loop-until-migrated=0 + exemption-set verification (§3.2 steps 3-4).** ONLY data-touching wave. | `run-envelope-migration.ts` (reuse + `--list-kinds`); shadow harness (§3.3); `docs/operator/envelope-substrate-cutover-runbook.md`; `SUBSTRATE_ENVELOPE_TOLERANT` strict-flip | W6-prep gate (§3.2 — incl. writer-closure re-verify + stuck-Message-40 root-cause); **shadow-read 100% parity (psql-oracle count + content-hash + every-renameMap-entry coverage) BEFORE flip**; **<60s TOTAL OBSERVED DOWNTIME — measured 47-57s composite at preflight, thin margin, image pre-pull mandatory** (§3.2); verification PASS (count parity + content-hash + zero legacy rows outside exemption set); rollback rehearsed on the clone | W1–W5 |
...
**Per-wave Hub-source PR cadence:** each wave is a `hub/src/` PR REQUIRING build-hub.sh + start-hub.sh + the Adapter-Restart-Protocol-includes-Hub-container discipline. Branch `agent-lily/m-envelope-substrate-completion-wN` (architect) ↔ `agent-greg/...` (engineer), cross-approval per `multi-agent-pr-workflow.md`; wave-per-PR cumulative-fold (mission-68 M6 pattern). Watchtower auto-deploy (bug-140) picks up each `build-hub.sh` push to AR `:latest` within ~5min — **the `:latest` push is the deploy trigger, NOT the git-merge** (no auto-build on merge), so production-dispositive follows the build-push, not the merge. (mission-90 W8, 2026-06-19: confirmed watchtower FUNCTIONAL live — W8 auto-deployed ~40s after the `:latest` push — and corrected the W6 cutover-runbook's stale "watchtower non-functional / deploy is MANUAL" claim, which contradicted this note. Operational consequence: a `:latest` push is a prod deploy, so never push `:latest` during a deploy-HOLD.)
...
**Open-item ledger (v1.1):**
1. ~~Tele-N pinning~~ **CLOSED at Manifest 2026-06-10** — §1: primary tele-1 + tele-7; secondary tele-2 + tele-6.
2. ~~Mission-rename ratification~~ **CLOSED at Manifest-bind 2026-06-10** — mission-90 spawned as M-Envelope-Substrate-Completion.
3. ~~W6 downtime preflight measurement~~ **CLOSED at Phase 6 preflight 2026-06-10** (thread-658) — 47-57s composite measured on a real prod-snapshot clone; <60s CONFIRMED-ACHIEVABLE; thin margin; image pre-pull mandatory. *(No open items remain; W6-prep re-verifications are wave-gate work, not design-open-items.)*

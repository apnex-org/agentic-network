# M-Envelope-Substrate-Completion — Design v1.1

**Status:** v1.1 — engineer round-1 audit integrated (thread-657 converged) + Director scope-in override for idea-318 (2026-06-10) + engineer W5 focused re-audit folded (self-trigger constraint → converge-then-stop semantics + runtimeLoop spec-equality guard + bounded-storm/restart-cycle/restart-survival gates; boot-failure WARN per Director). **Phase 5 Manifest BOUND: mission-90 spawned (saga-substrate-completion); idea-323/318/320/324 incorporated; tele pinned (primary tele-1 + tele-7; secondary tele-2 + tele-6).**
...
3. **W6 downtime preflight measurement** on a representative prod snapshot — at Phase 6 (§3.2). *(The ONLY remaining open item.)*

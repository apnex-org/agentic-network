# C1 — Sovereign Work-Control Plane — Arc Design (SURVEY-RESOLVED DESIGN CANDIDATE)

**Status:** SURVEY-RESOLVED — Phase-3 Director-intent Survey COMPLETE (2026-06-22; all 3 defaults confirmed, DR-S2-016). Supersedes the PR #347 DRAFT (run wf_9e7533bd-632). Integrates the convergences that were UNSETTLED at first authoring and are now DECIDED (D-1 R0 charter MERGED #348; D-3 telemetry = Option B Director-agreed DR-S2-012; C2 = headless-first + supervisor-owned-lifecycle DR-S2-013; C3-R4 renameMap-governor LIVE af17367). The 3 Survey calls are settled (see § "Survey questions — RESOLVED"). **Next: Phase-4 Design pass with greg (leads R2 construction-design) → ratify → bank R0–R4 missions.**

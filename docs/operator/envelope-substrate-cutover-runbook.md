...
- Prod Hub = `ois-hub-prod`, a COS container on GCE **`hub-vm`** (zone `australia-southeast1-a`), started by the Terraform **metadata `startup.sh`** via direct `docker run` (NOT docker-compose). Sibling containers on the `hub-net` docker network: `ois-postgres-prod` (the substrate), `watchtower-prod` (**FUNCTIONAL** — auto-deploys the AR `:latest` image, ~5min poll; see Deploy + the mission-90 W8 correction below).
...
- **Deploy mechanism — CORRECTED (mission-90 W8, 2026-06-19):** Watchtower IS functional. It polls AR `:latest` (~5min) and **auto-deploys** a newly-pushed image — so a `build-hub.sh` push to `:latest` IS a prod deploy (confirmed live: W8 auto-deployed ~40s after the `:latest` push, `Updated=1`). Consequences: (1) a "pre-build to `:latest`" is **NOT** a non-prod-mutating step — **do NOT push `:latest` during a deploy-HOLD** (near-miss: a hold-time pre-build would have auto-deployed W8 mid-hold). (2) A **merge to `main` alone does NOT deploy** — there is no auto-build on merge; only a `:latest` push triggers watchtower. The prior "Watchtower non-functional / deploy is MANUAL (can't auth to AR — W5/AG-W5.1)" claim was WRONG — it predates/contradicts the bug-140 watchtower-auth fix the Design §4 cadence note already records. Manual IAP-SSH recreate (`sudo docker stop/rm` + `google_metadata_script_runner startup`) remains the path for the **data-migration cutover** (the Hub must be stopped for the migration — watchtower can't orchestrate that) and as a **force-immediate / fallback override**. All `docker`/`psql` on hub-vm still run under `sudo` (`(ALL) NOPASSWD: ALL` for the IAP SSH user).

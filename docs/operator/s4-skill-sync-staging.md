...
2. Shallow-clones `apnex/mission-kit` and checks out the manifest's **pinned `source_ref` SHA** — so activation pulls the *reviewed* mission-kit state, not whatever `main` is at activation time. **Read-only.** Overrides: `SKILL_SYNC_REPO`/`SKILL_SYNC_REF` (testing only). The sync **logs the exact resolved SHA** (`… sha=<40-hex>`) so the `ois up` log records precisely what was deployed.

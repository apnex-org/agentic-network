...
- **bug-146 — FULLY resolved** (both axes confirmed live at W8: the dispatch `directive.substring` crash AND the caller-login-labeling claim-routing). BUT the thread-dispatch-equivalence workaround does **not** retire yet — the W8 live behavioral testing surfaced **bug-159 (major)**: there is no architect tool-path to dispatch an engineer-*claimable* task (`create_task` / `create_mission(plannedTasks)` inherit the creator's login-label with no assignee param → un-claimable by the engineer). bug-159 blocks the formal architect→engineer claim flow, so the retirement is **DEFERRED** until it lands (the clean post-mission-90 follow-on; outside the envelope scope).
  - **Meta-learning:** the live multi-agent co-drive earned its keep *twice* — it confirmed the bug-146 fixes AND surfaced bug-159, which the deep audit's testcontainer could not, because the testcontainer seeds claimable tasks directly and bypasses the login-label-inheritance dispatch path. A faithful unit harness still misses tooling-path bugs that only a real cross-agent dispatch exercises. (Candidate complement to calibration #4 faithful-harness-as-exposure: harness-fidelity catches shape bugs; live co-drive catches workflow-tooling bugs.)

...
**Cross-kind generality observation** (post-triage; surfaced via thread-632 bug-118 triage): the Defect "Lineage-substrate gap" instance progression (bug-118 + bug-27 + idea-side 80% analog) is retired by a **single envelope-level substrate fix that benefits all entity kinds uniformly** (Bug 0% → 100% capture; Idea 80% gap closed; Mission + Proposal + future kinds inherit). The kind-generality property is a **Concept-attribute axis distinct from Initiative-parentage** — substrate-generality (one fix; many kinds benefit) is structurally different from cross-leverage (one entity composing under N Initiatives). Both are prioritisation signals; v2.1 candidate K (substrate-generality as Concept-attribute) and v2.1 candidate L (high-leverage tier promotion when Tele alignment crosses threshold) name the two axes for future cross-leverage table tracking. The cross-kind generality finding makes bug-118 fix the **highest-leverage W1 substrate work** — the multi-kind-benefit-per-substrate-touch ratio is dispositive at wave-Survey scheduling.

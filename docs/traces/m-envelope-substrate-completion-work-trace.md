...
### Slice 2 — empirical run on the real prod snapshot (2026-06-19; Director-authorized read-only)

Director authorized read-only IAP-SSH (option b). Acquired a fresh read-only snapshot (`pg_dump -Fc -U hub_reader -t entities`, 12.3MB, ZERO mutation) → local throwaway clone → all analysis on the clone (no writes/cursor-advance/Hub-stop/deploy on prod). Snapshot + clone shredded/removed after. Full results in the readiness-report ADDENDUM (E1–E7); the run was DECISIVE:

- **27 rows dirty-cursor-TRAPPED in prod RIGHT NOW** (Bug 17 @ cursor bug-99, Task 6 @ task-97, Idea 4 @ idea-54). Validated: `--reset-checkpoints` + re-migrate recovers all 27 → envelope. (bug-154, hard numbers.)
- **NEW data-loss bug found + FIXED — migration OFFSET-PAGINATION silent-skip** (= the preflight "stuck-Message", root-caused): `runKind` listed `{limit,offset}` with NO sort → postgres unstable heap order + in-loop `put()` mutations → the offset window silently skips ~6-7%/pass (empirically 104: Message 85, Audit 19; rowsErrored=0); the cursor advances past them so loop-to-0 terminates WITH bare rows = data-loss at the strict-flip. Pre-existing (every migration incl. 2026-05-25). FIX: `runKind` lists `sort:[{field:'id',order:'asc'}]` (stable; migrated rows keep id). VALIDATED on real data: single pass migrated ALL 1491 bare, 0 errored, **0 bare outside exemptions**. Architect to file the bug + assign a number.
- **TIMING (real data, fixed):** re-migration ~40s; composite ~66-75s → OVER the <60s budget (1488 bare now vs preflight 686). Decision before GO: §R11 parallel lever / pre-cutover backlog reduction / accept ~70s.
- **bug-151/152 liveness CORRECTED to LATENT** (0 scheduled pending; 653 envelope threads/0 active; 13 teles/0 active → 0 currently-affected). Earlier "broken today" was an overclaim; the empirical check corrected it.
...
### Slice (post-audit) — deep audit → merge → redeploy (2026-06-19/20)

- **Deep adversarial audit (7 verifiers + completeness-critic):** verdict gaps-found / ZERO BLOCKERS; core verified sound (19 CAS decode-before-transform, every domain-kind decode, 3 helpers gone, anti-false-green core holds, Task createdAt/createdBy leaf-preserving → data-location caveat REFUTED). 2 majors + 5 minors + adapter-note → audit-response commit 7e63b63 (MG1 = DELETE the 4 dead `*-sweeper-substrate` W3.x spikes [8 files]; MG2 = wire setFieldTranslator+setWriteEncoder into message/turn/agent/audit postgres repo tests → real-pg envelope decode now exercised; m1–m5 + adapter scope). Suite green 1925/7-skip.
- **Re-audit CLEAN + MERGED:** PR #323 squash-merged to main = **7550cfd**. bug-138 read-side STRUCTURALLY CLOSED; idea-327 incorporated (architect, RACI).
- **Authorization-gate catch:** architect directed the code-only redeploy ("go when ready"); the recreate is an IAP-SSH PROD-WRITE → surfaced that it needs EXPLICIT Director auth (per standing constraint + bug-158 Director-ruled precedent). Architect confirmed they'd GLOSSED it ("CORRECT gate"); **Director said HOLD ('hold for now')**. Stood down clean (pre-build NOT kicked — HOLD landed first). Captured as memory [[feedback_prod_write_director_auth_gate]].
- **Director lifted the HOLD (direct) → redeploy executed 2026-06-20:**
  - Build off merged-main (7550cfd; hub/ content-identical to my checkout, verified) → `CI=1 OIS_ENV=prod build-hub.sh` → AR `hub:latest` = **`sha256:0294ddc5510e74cdaeaf680d025847cd0e21071edc4b89294bba7080aa6cc2c4`** (W8). Rollback target recorded BEFORE the push: pre-W8 `:latest` = `sha256:267548d7d4fc7ad3b85bf35d13b169b1915535d195440963f4cb21373e4aae38` (edc4792-era).
  - **WATCHTOWER AUTO-DEPLOYED** the new `:latest` ~40s after the push (23:29:15–25Z; 401-challenge → fallback pull → recreate; `Updated=1`, ~5-min poll) — so the manual `google_metadata_script_runner startup` recreate was UNNECESSARY and NOT run. **This contradicts the "watchtower non-functional / manual deploy" model** (memory + runbook B.3 + the architect's plan) — a build-push IS a prod deploy; a near-miss vs the just-lifted HOLD. Corrected memory [[hub-prod-runs-on-hub-vm-gce-local-docker-is-dev-test-only]]; runbook B.3 + Design W-row need architect correction.
  - **Boot-verify PASSED:** live image RepoDigest = W8 `0294ddc`; log `envelope substrate: STRICT (… retired at mission-90 W8)`; reconciler 23/23 (0 failures); 71 tools; MCP Relay listening :8080; 0 boot errors; container stable.
  - **Post-deploy LIVE AUDIT — MCP-GATED (deferred):** this session's MCP client is disconnected, so the dispositive checks (bug-146 dispatch live-half, caller-login-labeling, the 5 prod-defects-live, 9/9 list parity vs psql oracle) cannot run yet. Prod Hub relay IS up → run on MCP reconnect. bug-146/idea-325/bug-156-resolve remain post-live-audit.

**Status:** W8 LIVE on prod (image 0294ddc, boot-verify passed) via watchtower auto-deploy off the Director-authorized build-push.

### Slice — adapter outage + post-deploy LIVE AUDIT (2026-06-20)

- **Adapter outage (resolved):** the session's stdio shim was down ~10h — FATAL on a missing `OIS_AGENT_NAME` after the session lost the env (Claude Code does not auto-reconnect stdio MCP servers). Root-caused (shim log) + fixed by relaunching `claude` with `greg.env` sourced. Captured as memory [[reference_adapter_stdio_no_autoreconnect]].
- **Read-only live-audit = GREEN:** 9/9 list-parity vs the psql oracle (Mission 1/90, Bug-open 59, Idea-open 217, Task-working 7, Thread-active 0, Tele 13, Proposal 32 — all match) AND every list result is decode-to-flat (status STRING not {phase}; fields flat; tags arrays). bug-138 read-side + idea-325 (list_missions) + idea-327 (output-shape) CONFIRMED LIVE + correct on real prod data.
- **Behavioral live-audit** (Director-authorized full set; architect+Director watching; co-driven — engineer lacks create_task/cancel_task/create_review/create_mission/close_thread authz):
  - **#1 bug-146: PASS (both axes)** — dispatch-half (task-422 directive decoded INTACT, no dispatchTaskSpawned `.substring` crash) + caller-login-labeling (task-422 inherited apnex-lily label → greg correctly can't claim → label-scoped claim routing RESTORED post-decode).
  - **#2 proposal-close: PASS** — close_proposal(prop-33) → implemented (close CAS reads decoded status; bug-138 §1.10).
  - **#3 message-seq: PASS** — thread-659 sequenceInThread 0,1,2,3 monotonic (allocateSequence; bug-138 §1.1).
  - **#4/#5/#6 (3-strikes / mission-auto-advance / PA-watchdog): BLOCKED by bug-159** — no architect tool-path dispatches a greg-CLAIMABLE task (create_task + create_mission(plannedTasks) both inherit the architect's apnex-lily login-label + no assignee param → un-claimable by greg), so the positive-claim path can't be exercised live. These rest on the deep-audit testcontainer positive-proofs (real-PG round-trip) + the live read-decode. Sufficient.
- **thread-dispatch-equivalence retirement = DEFERRED** (architect call): bug-146 (crash + label-decode) FIXED + live-confirmed, but bug-159 blocks the formal architect→engineer-claim flow end-to-end; the workaround stays until bug-159 (assignee/label-scoping) lands.
- **Ledger:** bug-156 resolved (7550cfd); bug-157 satisfied; idea-327 incorporated; **bug-159 FILED (major)** — the residual formal-dispatch blocker, the clean post-mission-90 follow-on. Cleanup: mission-91 abandoned, thread-659 closed, task-422 cancelled, prop-33 closed.

**Final status:** mission-90 PRIMARY OUTCOME (bug-138 structural close) DONE + LIVE-VALIDATED on prod. Behavioral set: 3/3 testable PASS; #4/#5/#6 blocked by bug-159 (covered by the testcontainer positive-proofs + live read-decode). thread-dispatch-equivalence retirement deferred to bug-159's fix. Architect finalizing the Phase 7 release-gate §6 + Phase 10 retro + the runbook B.3 / Design W-row watchtower corrections (branch m90-release-gate-retro); this work-trace lands standalone (cross-approve per RACI). bug-159 = the follow-on.

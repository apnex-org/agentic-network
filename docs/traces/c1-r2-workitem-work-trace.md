...
- ✅ **958df36 — sub-PR-4b-ii thrash-quarantine wiring (C1→C2 seam):** sweeper increments the holder on claim→expire-without-evidence; claim_work locks out a quarantined agent; complete_work resets on success; clear_work_quarantine ([Architect|Director]) = the R2 manual escape.
  - ⚠️ **SUPERSEDED 2026-07-28 by `work-593` (idea-675) — the entry above is left VERBATIM because it is a true record of what `958df36` shipped, not a description of current behaviour.** The `[A]` agent claim-thrash lockout and `clear_work_quarantine` are **REMOVED**: `claim_work` no longer locks anyone out, and the verb is unregistered. **`thrashCount` still writes** (Director ruling: useful as a metric) and still resets on a successful `complete_work`. `Agent.quarantined` remains a stored field with **no writer and no reader**. Reason for removal: the lockout **suppressed the driver liveness alarm for exactly the seats it trapped** — see `docs/verification/work-593-quarantine-removal.md`.

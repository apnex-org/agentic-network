...
| Commit-push thread-heartbeat mechanization (mission-68 closure of #55; mission-85 Layer (c) retirement) — **20min engineer / 40min architect escalation horizon** post-mission-68 (10/20 cadence × missedThreshold=2; 2.25× faster than v1.2 baseline; budget for ~30min deep-thought-phases on substrate-introduction work) | 2-layer stack post mission-85: (a) methodology-doc fold here + (b) adapter-side commit-push hook (Bash-tool-result post-process; NDJSON observability emit). Layer (c) Hub-side handler RETIRED per CROSS-LAYER IDENTITY EXTRACTION calibration (bug-98 wontfix); architect-side PR-review-readiness signal achieved via engineer-thread-explicit-surface on coord-thread | `mission-lifecycle.md` | [§1.5.1.1 Commit-push thread-heartbeat mechanization](mission-lifecycle.md#1-5-1-1-commit-push-thread-heartbeat-mechanization-mission-68-w1-closure-of-55-mission-85-retirement-of-layer-c) |
...
| Envelope substrate STRICT decode-to-flat (post-mission-90) | Storage holds envelope rows only (no legacy-flat write path); every repo decodes envelope→flat on `get`/`list`/`findBy*` AND inside `casUpdate`/`tryCasUpdate` (transform on flat → write-encoder re-envelopes); read flat fields directly above the membrane and do NOT reintroduce a dual-shape reader — the full runtime contract is restated here so cold-pickup needs no further hop before touching a repo | `CLAUDE.md` (Tier 0) | [§Envelope substrate — STRICT + decode-to-flat](../../CLAUDE.md#envelope-substrate--strict--decode-to-flat-post-mission-90) |

...
- **P0 — Charter & Calibration.** Lily + Greg draft; Director ratifies (Phase-3-survey style; **verdict-neutral**). Establishes: the **frozen snapshot 4-tuple** (B.5); the **live axiom-set** dumped to a held artifact (NOT re-queried — `list_axioms` serves the synced snapshot); the per-cell **verdict schema** `{axiom, PASS|PARTIAL|FAIL, evidence[], evidence-tier, severity, remediation, self-confidence}`; the **dual proof-bar + per-axiom harness table** (B.6); the **severity rubric** anchored to Director-impact-on-blast-radius (B.6).
...
- **P9 — Converge & materialize.** `close_thread` converged=true, summary = the ratified drift-matrix; cascade `create_audit_entry` (per verdict + surviving evidence + dissent), `create_bug` (per material DUAL-TRUTH/FAIL), `create_idea` (remediation / UNAUDITED-waiver / tele-mis-spec). **Constitutional changes (axiom amendments) ride the mission-kit pen (author + gauntlet + Director merge on `axioms/`), not a Hub verb → a close whose cascade would touch the constitution files a follow-on rather than mutating in-Hub.** Emit convergence-quality metrics (resolved/forwarded, canary recall+precision, drift-rate).
...
2. **axiom-data-dump** — `list_axioms` serves the synced constitutional snapshot; dump it to a held artifact at P0 so both principals audit the *same* set.

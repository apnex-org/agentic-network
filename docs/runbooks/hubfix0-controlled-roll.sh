...
emit_status(){ # $1=disposition
  ...
  json=$(jq -n \
    ...
    '{node:"hubfix0-ROLL", disposition:$disp, terminal_reads_ok:$ro, script_self_sha256:$self,
      ...
      decision_ref:$dr}') || { echo "[roll] FATAL: jq render failed -- EVIDENCE INCOMPLETE" >&2; return 1; }
  # V7-5: genuine create-once at the single terminal emission (noclobber; umask 077 already set)
  ( set -o noclobber; printf '%s\n' "$json" > "$STATUS_OUT" ) 2>/dev/null || { echo "[roll] FATAL: STATUS_OUT create-once emission failed: $STATUS_OUT" >&2; return 1; }
  rec "TERMINAL disposition=$1 reads_ok=$reads_ok status=$STATUS_OUT"
  return 0
}
die(){ log "FATAL: $*"; emit_status "preflight_or_fatal" || true; exit 1; }

# ── jq required (STATUS_OUT is created create-once at emission by emit_status — V7-5) ──
...
forward(){
  ...
  [ "$(wt_running)" = "false" ] || { FAIL="watchtower-not-stopped-final"; return 1; }   # V7-4 fresh re-assert
  return 0
}
if forward; then
...
PGR=$(pg_invariants)
if $RB_OK && [ "$PGR" = "$PG0" ] && [ "$WATCHTOWER_STOPPED" = "true" ] && [ "$(wt_running)" = "false" ]; then   # V7-4 fresh watchtower re-assert

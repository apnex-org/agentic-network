...
Review-request subkinds enter the Hub as event/note feed inputs. `pr-review-requested` means GitHub emitted an explicit review-request assignment event; `pr-review-request-removed` is cancellation metadata. At the rule boundary the Hub may normalize these legacy subkinds to upstream-aligned event names (`github.pull_request.review_requested`, `github.pull_request.review_request_removed`) and evaluate the static `pr_review_request_to_workitem_v0` rule. Denied/unbound/ambiguous/team/stale cases remain fallback/candidate notes with raw drill-down and an explicit fallback reason. Neither raw subkind is branch-protection truth, a WorkGraph obligation by itself, CODEOWNERS/team membership expansion, merge actuation, full programmable rules-engine promotion, or implementation-work auto-completion.

...
// ─────────────────────────────────────────────────────────────────────────────
// 🔴🔴 SKIPPED — AND THE SKIP IS THE FINDING, NOT A TODO.
//
// THE REMEDY THESE ASSERTIONS DEMAND IS DISPROVEN AS A FAMILY. Five variants were
// tested against the real transport — requestInit.dispatcher, setGlobalDispatcher at
// creation, setGlobalDispatcher at teardown-only, and timeouts-zeroed — and ALL FIVE
// HANG on connect. The bare `import { Agent } from "undici"` is innocent; it is
// REPLACING UNDICI'S DISPATCHER AT ALL that breaks the SSE request path.
//
// So `:dispatcher: this.httpDispatcher` below is not an unimplemented goal. It is the
// EXACT CONSTRUCT MEASURED TO HANG. These assertions are retained, skipped, as the
// record of a hypothesis that was tested and rejected — NOT as acceptance criteria.
// DO NOT UN-SKIP THEM AND DO NOT IMPLEMENT WHAT THEY ASSERT.
//
// THE CONSTRAINT ANY REAL FIX MUST SATISFY: pool replacement must not touch the SSE
// request path. Mechanism UNKNOWN. The teardown-only variant is the lead.
//
// The three behavioural tests ABOVE are unaffected and remain live: they exercise
// undici directly, reproduce the failure against real sockets, and carry their own
// discriminator. The reproduction stands on its own; only the src-wiring claim falls.
// ─────────────────────────────────────────────────────────────────────────────
describe.skip("bug-420: the pool-per-wire fix is wired into McpTransport, not just proven in principle", () => {

...
```bash
npm install --global @apnex/claude-plugin@0.1.18 --ignore-scripts
...
The package's `.claude-plugin/marketplace.json`, `.claude-plugin/plugin.json`, and npm `package.json` all carry version `0.1.18`. `dist/identity.json` and `dist/member-manifest.json` expose the exact built source and installed-file projection used by protected release verification.

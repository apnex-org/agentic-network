/**
 * mission-102 P3-B2 — RawDecisionRaised KindMigrationModule (WRITE-side envelope
...
const KIND = "RawDecisionRaised";

export function createRawDecisionRaisedMigrationModule(schema: SchemaDef): KindMigrationModule {
  const schemaRef: MigrationSchemaRef = {
    schema,
    renameMap: {},
    partition: {
      metadata: ["createdAt", "updatedAt"],
      spec: ["decisionId", "title", "context", "class", "options", "contextRefs", "raisedBy", "raisedAt"],
      status: [],
    },
  };
  ...
  return {
    ...
    migrateOne(legacy: unknown): unknown {
      ...
      if (typeof legacy !== "object" || legacy === null) {
        throw new Error(`[RawDecisionRaised.migrateOne] input must be object, got ${typeof legacy}`);
      }

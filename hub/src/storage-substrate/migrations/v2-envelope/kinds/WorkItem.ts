...
export function createWorkItemMigrationModule(schema: SchemaDef): KindMigrationModule {
  const schemaRef: MigrationSchemaRef = {
    ...
    partition: {
      ...
      status: ["lease", "evidence", "frictionReflections", "blockedOn", "leaseExpiryCount", "enteredCurrentStateAt", "stateDurations", "attestationHistory", "attestations", "executorHistory", "recallHistory", "pendingRecallIntents", "failedGateSeal", "pendingFailedSealNotices", "failedSealNoticePending", "effectiveDisposition", "nodeConfig"],
    },
  };

...
function cloneFlat<T>(row: T, kind: string): T {
  // ConstitutionSnapshot has a domain field named `status` which the canonical
  // envelope stores at status.status. The generic decoder necessarily reserves
  // top-level `status` for status.phase and therefore drops this co-located leaf.
  // Capture and restore the domain value at this repository boundary only; do
  // not weaken the generic phase decoder for every other envelope kind.
  const raw = row as unknown as Record<string, unknown>;
  const statusBucket = raw.status !== null && typeof raw.status === "object" && !Array.isArray(raw.status)
    ? raw.status as Record<string, unknown>
    : null;
  const domainStatus = kind === SNAPSHOT_KIND && typeof statusBucket?.status === "string"
    ? statusBucket.status
    : undefined;
  const flat = decodeEnvelopeToFlat(raw, kind) as unknown as Record<string, unknown>;
  if (domainStatus !== undefined) flat.status = domainStatus;

...
describe("ClassGrant (real-pg: mint / revoke / evaluate / the gate's grant path)", () => {
  ...
  async function routedDecision(cls: string | null, plan?: DecisionPlanAction[]) {
  ...
  it("mint mechanics: ratification fail-closed; reversibleOnly self-contradiction rejects; version chain via supersede", async () => {
    ...
    const g2 = await mintApprovalUnblock({ supersedes: g1.id, ratificationRef: "decision-ratify-2" });
  ...
  it("contract #1: a decision OUTSIDE the grant's class rejects — exact-match, excluded-class belt, and unclassified all fail closed", async () => {
    ...
    // the verifier's lean-laundering scenario: a scope-change curated under a
    // different label still cannot ride — exact-match fails first…
    const spoofed = await routedDecision("scope-change", [{ action: "approve", targetRef: "prop-1" }]);
    ...
    // …and even a decision RE-CLASSED to the covered label is caught by the
    // excluded-class belt when curation later corrects it (defense in depth is
    // exercised at the evaluator level):
    const excluded = { ...(await routedDecision("approval-unblock")), class: "scope-change" };
    expect(() => evaluateGrant(g, excluded, new Date().toISOString())).toThrow(/not 'scope-change'/);
    // unclassified NEVER rides a grant
    const unclassified = await routedDecision(null);
    await expect(decisions.resolveDecision(unclassified.id, ARCHITECT, { chosenOptionId: "a" }, gate, { claimedAuthorityRef: g.id }))
  ...
  it("contract #3: a REVOKED grant authorizes nothing new; a prior resolution retains its exact id@version; revoked rows keep full content", async () => {
    const g = await mintApprovalUnblock();
    const d1 = await routedDecision("approval-unblock", [{ action: "unblock", targetRef: "work-1" }]);
    ...
    // nothing NEW flows
    const d2 = await routedDecision("approval-unblock", [{ action: "unblock", targetRef: "work-2" }]);

...
export function registerWorkItemPolicy(router: PolicyRouter): void {
  ...
  router.register(
    "attest_evidence",
    "[Verifier] SEAL (idea-444): record a load-bearing pass/fail ATTESTATION against a verifier-attestation requirement — the authority-separated verdict that retires the audit-as-verdict path. verifierId is SERVER-STAMPED from the session (caller cannot forge it). Rejects: a non-verifier-attestation requirement; evidenceRefs that fail typed validation or lack a load-bearing ref (see below); a verifierId in the VERIFIED WORK's executor/holder/creator history (self-attestation — closes release-then-attest; idea-528 permits a verifier to mechanically drive a gate node for work they did not author; unresolvable target work fails safe to gate-node history); a targetRef that moved since an attestation exists (relocation laundering). All row-derived checks (hashes, history, ref-relatedness) run against the FRESH row under CAS. Appends to the append-only attestationHistory + repoints the active projection (preserve-not-inject MERGE). DUAL-EDGE: for a LEAF item parked in review, a passing gate advances review→done in the same write (a GATED-ARC node still completes only via complete_work, which re-checks its completionDependsOn). A `fail` verdict keeps it in review (a later `pass` supersedes + unparks).",
    {
